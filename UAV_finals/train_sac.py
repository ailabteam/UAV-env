...
if __name__ == '__main__':
    run_name = f"SAC_v7_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    checkpoint_callback = CheckpointCallback(save_freq=max(CHECKPOINT_FREQ // NUM_ENVS, 1), save_path=model_dir, name_prefix="sac_model")
    device = "cuda" if torch.cuda.is_available() else "cpu"; print(f"Using device: {device}")
    model = SAC("MlpPolicy", vec_env, device=device, verbose=1, tensorboard_log=log_dir)
    print(f"--- Starting SAC Training on Env v7 ---")
    model.learn(total_timesteps=TRAIN_TIMESTEPS, callback=checkpoint_callback, progress_bar=True)
    model.save(f"{model_dir}/sac_model_final.zip")
