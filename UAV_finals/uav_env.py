...
class UAVNetworkEnv(gym.Env):
    """
    v7.2: Phiên bản cuối cùng cho thí nghiệm, đã kiểm tra kỹ `force_state`.
    """
    metadata = {'render_modes': ['human']}

    def __init__(self, num_uavs=2, num_iot_nodes=15):
        # __init__ và _get_obs giữ nguyên như v7
    ...
    def _get_obs(self):
        ...
        for uav in self.uavs:
            ...
            # Sắp xếp IoT theo khoảng cách để observation nhất quán
            sorted_iots = sorted(self.iot_nodes, key=lambda i: np.hypot(i['x'] - uav['x'], i['y'] - uav['y']))
            iot_states = []
            for i in range(N_CLOSEST_IOT):
                if i < len(sorted_iots):
                    iot = sorted_iots[i]; iot_states.extend([(iot['x']-uav['x'])/WORLD_SIZE_X, (iot['y']-uav['y'])/WORLD_SIZE_Y, iot['battery']/IOT_BATTERY_START, iot['data']/IOT_DATA_START])
                else: iot_states.extend([0,0,0,0])
            sorted_nfzs = sorted(self.no_fly_zones, key=lambda z: np.hypot(z['center'][0] - uav['x'], z['center'][1] - uav['y']))
            nfz_states = []
            for i in range(N_CLOSEST_NFZ):
                if i < len(sorted_nfzs):
                    zone = sorted_nfzs[i]; nfz_states.extend([(zone['center'][0]-uav['x'])/WORLD_SIZE_X, (zone['center'][1]-uav['y'])/WORLD_SIZE_Y, zone['radius']/MAX_ZONE_RADIUS])
    ...
    def force_state(self, uavs, iot_nodes, no_fly_zones):
        """Ép môi trường phải bắt đầu từ một trạng thái cụ thể và trả về obs ban đầu."""
        self.current_step = 0
        self.uavs = copy.deepcopy(uavs)
        self.iot_nodes = copy.deepcopy(iot_nodes)
        self.no_fly_zones = copy.deepcopy(no_fly_zones)
        return self._get_obs() # Chỉ trả về obs, không có info

    def step(self, action):
        # ... (Hàm step giữ nguyên như các phiên bản trước) ...
