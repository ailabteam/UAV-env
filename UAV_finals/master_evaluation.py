...
import os, re, copy, pandas as pd, numpy as np
...
if __name__ == '__main__':
    ...
    for run_idx in range(NUM_TOTAL_RUNS):
        ...
        for episode_idx in range(NUM_EPISODES_PER_RUN):
            ...
            # 2. CHẠY TẤT CẢ PHƯƠNG PHÁP TRÊN CÙNG KỊCH BẢN
            for name, policy_info in policies_to_evaluate.items():
                
                print(f"    -> Đang đánh giá phương pháp: {name}")

                if policy_info['is_rl']:
                    r, d, traj = run_single_episode(None, initial_state, is_rl=True, model_path=policy_info['model_path'], algo_class=policy_info['algo_class'])
                else:
                    r, d, traj = run_single_episode(policy_info['policy_func'], initial_state)
                ...
                if episode_idx == NUM_EPISODES_PER_RUN - 1: 
                    plot_trajectory(traj['trajectories'], [(n['x'], n['y']) for n in initial_state['iot_nodes']], initial_state['no_fly_zones'], f"{output_dir}/{name}")
    ...
    summary = df.groupby('policy').agg(
        ...
        avg_data=('data', 'mean')).sort_values(by='avg_reward', ascending=False)
