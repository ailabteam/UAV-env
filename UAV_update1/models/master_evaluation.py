...
def run_single_episode(policy, initial_state, is_rl=False, model_path=None, algo_class=None):
    ...
    if is_rl:
        ...
        model = algo_class.load(model_path, env=vec_env, device='cpu')
        obs = vec_env.reset()
    else:
        obs, _ = env.reset()
    ...
    while not done:
        if is_rl:
            ...
            episode_reward += reward; done = terminated or truncated
        ...
        if done:
            ...
            if final_info:
                data_collected = (env.num_iot_nodes * IOT_DATA_START) - final_info['iot_data_left']
            else:
...
def find_latest_model(algo_name_prefix):
    """Tìm model mới nhất cho một thuật toán, không phân biệt hoa/thường."""
    try:
        models_dir = 'models'
        # ======================== SỬA LỖI Ở ĐÂY ========================
        # Chuyển cả tên thư mục và prefix sang chữ thường để so sánh
        algo_dirs = [d for d in os.listdir(models_dir) if d.lower().startswith(algo_name_prefix.lower())]
        # =============================================================
        if not algo_dirs:
            print(f"Không tìm thấy thư mục nào bắt đầu bằng '{algo_name_prefix}'")
            return None
        ...
        if not model_files:
            print(f"Không có file .zip nào trong {latest_dir_path}")
            return None
        ...
        print(f"Lỗi khi tìm model cho '{algo_name_prefix}': {e}")
        return None
...
if __name__ == '__main__':
    ...
    # ======================== SỬA LỖI Ở ĐÂY ========================
    # Sửa lại prefix để khớp chính xác với tên thư mục
    tqc_path = find_latest_model("TQC_v7")
    sac_path = find_latest_model("SAC_v7")
    # =============================================================
    ...
    for run_idx in range(NUM_TOTAL_RUNS):
        ...
        for episode_idx in range(NUM_EPISODES_PER_RUN):
            ...
            for name, policy_info in policies_to_evaluate.items():
                ...
                is_rl = policy_info[1]
                if is_rl: 
                    r, d, traj = run_single_episode(None, initial_state, is_rl=True, model_path=policy_info[0], algo_class=policy_info[2])
                else: 
                    r, d, traj = run_single_episode(policy_info[0], initial_state)
    ...
    print("\n\n" + "="*80)
    print("--- BẢNG TỔNG KẾT THÍ NGHIỆM CUỐI CÙNG (TRUNG BÌNH QUA TẤT CẢ CÁC LẦN CHẠY) ---")
    print("="*80)
