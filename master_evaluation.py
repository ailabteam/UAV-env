...
# ======================== ĐÃ SỬA LỖI IMPORT (KIỂM TRA LẠI) ========================
...
# ==============================================================================
# PHẦN 1: CÁC HÀM ĐÁNH GIÁ (LOGIC ĐÃ SỬA LỖI)
# ==============================================================================
...
def run_single_episode_rl(model_path, algorithm_class, initial_state):
    ...
    while not done:
        ...
        if done:
            # final_info is available from info dictionary when an episode terminates
            final_info = info.get('final_info')
            if final_info is not None:
                final_data_left = final_info.get('iot_data_left', sum(iot['data'] for iot in eval_env.get_attr('iot_nodes')[0]))
                data_collected = (eval_env.get_attr('num_iot_nodes')[0] * IOT_DATA_START) - final_data_left

    # Fallback if final_info is not available (e.g., truncated)
    if data_collected == 0:
